...
simplify.enrichment =
  function(enrichment.results,
    ...
    ## Reapply enrichment analyses
    new_analyses = DEprot::geneset.enrichment(DEprot.analyses.object = dpo.analyses.object,
                                              contrast = contrast.idx,
                                              TERM2GENE = geneSet_tb,
                                              enrichment.type = enrichment.results@parameters$enrichment.type,
                                              gsea.rank.method = enrichment.results@parameters$gsea.rank.method,
                                              diff.status.category = enrichment.results@parameters$diff.status.category,
                                              gsub.pattern.prot.id = enrichment.results@parameters$gsub.pattern.prot.id,
                                              pvalueCutoff = enrichment.results@parameters$pvalueCutoff,
                                              qvalueCutoff = enrichment.results@parameters$qvalueCutoff,
                                              pAdjustMethod = enrichment.results@parameters$pAdjustMethod,
                                              dotplot.n = enrichment.results@parameters$dotplot.n)

    new_analyses@affinity.propagation = clusters
