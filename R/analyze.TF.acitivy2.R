...
analyze.TF.activity =
  function(sample.groups,  # sample.ID | group
    ...
    if (!file.exists(aggregate.scores.dir)) {
      return(warning("The aggregate scores directory does not exist!"))
    }
    ...
    if (length(TF_list) == 0){return(warning("No TF to be analyzed. Possibile reasons:\na) the suffix of the aggregate files is incorrect\nb) upon balcklist/whitelist filtering no TFs are left"))}
    ...
    if (verbose == T){message("Read score tables...")}
    ...
    if (verbose == T){message("\n\nCompute statistics and means for each TF...")}
    ...
    pb1 = utils::txtProgressBar(min = 0,      # Minimum value of the progress bar
                                max = length(TF_list), # Maximum value of the progress bar
    ...
    for (i in 1:length(TF_list)) {
      ...
      if (verbose == T){utils::setTxtProgressBar(pb1, i)}
    } # END
    ...
    # ---------------------------------------------------------------------------------------------------
    # Filter foot prints
    if (filter.footprints == TRUE) {
      if (verbose == T){message("\n\nFiltering footprints...")}
      
      mean.sd_ratio_tb =
        do.call(rbind,
                lapply(reshaped_score_mean_list, function(x){
                  tb =
                    dplyr::group_by(x, group) %>%
                    dplyr::summarise(mean = mean(score_mean, na.rm = T),
                                     median = median(score_mean, na.rm = T),
                                     sd = sd(score_mean, na.rm = T)) %>%
                    dplyr::mutate(mean.sd_ratio = mean/sd)
                  
                  return(data.frame(TF = "TF",
                                    mean.sd_ratio = max(tb$mean.sd_ratio, na.rm = T)))
                }))
      mean.sd_ratio_tb$TF = rownames(mean.sd_ratio_tb)
      
      TF_selection = mean.sd_ratio_tb$TF[mean.sd_ratio_tb$mean.sd_ratio >= min_mean.sd.ratio]
    } else {
      TF_selection = names(reshaped_score_mean_list)
    }
    ...
    pb2 = utils::txtProgressBar(min = 0,      # Minimum value of the progress bar
                                max = length(TF_selection), # Maximum value of the progress bar
                                style = 3,    # Progress bar style (also available style = 1 and style = 2)
                                width = 50,   # Progress bar width. Defaults to getOption("width")
                                char = "\u25a0")   # Character used to create the bar
    ###
    ...
    for (i in 1:length(TF_selection)) {
      # table by sample
      AUC_bySample[[i]] =
        reshaped_score_list[[TF_selection[i]]] %>%
      ...
      # print progression bar:
      if (verbose == T){utils::setTxtProgressBar(pb2, i)}
    }
    names(AUC_byGroup) = TF_selection
    names(AUC_bySample) = TF_selection
    ...
    for (j in 1:length(comparison.list)) {
      ...
      quantiles = quantile(FC_table_list[[j]]$`difference.mean.AUC.A-B`,
                           c(percentage.top.hits/100, 1-(percentage.top.hits/100)),
                           na.rm = T)
      
      Diff_MAplot_list[[j]] =
        ggplot(data = FC_table_list[[j]] %>% dplyr::filter(`difference.mean.AUC.A-B`>quantiles[1] & `difference.mean.AUC.A-B`<quantiles[2]),
               aes(x = basemean.AUC,
                   y = `difference.mean.AUC.A-B`)) +
        geom_point(size = 2, alpha = 0.5, show.legend = F, color = "gray50") +
        geom_point(data = FC_table_list[[j]] %>% dplyr::filter((`difference.mean.AUC.A-B`<=quantiles[1] | `difference.mean.AUC.A-B`>=quantiles[2]) & (abs(basemean.AUC) > basemean.AUC.threshold)),
        ...
        xlab(paste0("mean AUC (", comparison.list[[j]][1], ":", comparison.list[[j]][2], ")")) +
        ylab(paste0("AUC difference: ", comparison.list[[j]][1], " - ", comparison.list[[j]][2])) +
        geom_hline(yintercept = log2(1), linetype = "dotted", color = "gray50") +
      ...
      if (MAplot.show.names == TRUE) {
        Diff_MAplot_list[[j]] =
          Diff_MAplot_list[[j]] + 
          ggrepel::geom_text_repel(data = FC_table_list[[j]] %>% dplyr::filter((`difference.mean.AUC.A-B`<=quantiles[1] | `difference.mean.AUC.A-B`>=quantiles[2]) & (abs(basemean.AUC) > basemean.AUC.threshold)),
                                   aes(x = basemean.AUC,
                                       y = `difference.mean.AUC.A-B`,
                                       label = TF),
                                   color = "#07c22c",
