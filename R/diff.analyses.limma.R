...
diff.analyses.limma =
  function(DEprot.object,
    ...
    for (i in 1:length(contrasts.info)) {
      ...
      scatter.PC1.2 = DEprot::plot.PC.scatter(DEprot.PCA.object = PCA.data, PC.x = 1, PC.y = 2, color.column = contrasts.info[[i]]$metadata.column, shape.column = switch(isTRUE(include.rep.model)+1, NULL, replicate.column), plot.zero.line.x = FALSE, plot.zero.line.y = FALSE) + geom_hline(yintercept = 0, colour = "gray", linetype = 2) + theme(legend.position = "none")
