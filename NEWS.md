...
- `expression.boxplot()`: the Wilcoxon p-values follow the defaults of `wilcox.test()`, i.e. exact whenever they can be computed. The normal approximation (`exact = FALSE`) made the comparisons between few replicates look stronger than they are: two groups of 3 samples that do not overlap gave p = 0.081, and 0.0495 (one star) with `pairwise.test.type = "kruskal.test"`, which dropped the continuity correction as well, while the exact p-value cannot go below 0.1
- `expression.boxplot()` gains `paired.test` and `replicate.column`, which work as in `diff.analyses()`: the samples are matched between groups through their replicate ID, the t-tests become paired t-tests and the Wilcoxon test a signed-rank test. With more than two groups the global p-value comes from a repeated-measures ANOVA or from a Friedman test. `pairwise.test.type = "paired t-test"` was accepted, but it ran an unpaired t-test: it now performs the paired test and requires replicate.column
- in `expression.boxplot()` all the p-values, global label and significance symbols included, are computed by the function itself and drawn with `ggpubr::stat_pvalue_manual()`: they no longer depend on the ggpubr version installed (the compare_means() of ggpubr 0.6.2 sets `exact = FALSE` when it is not specified), and with `pairwise.include.ns = FALSE` the non-significant brackets are removed for both label styles. With several proteins the global label is placed above the brackets of its own panel
- updated overview vignette, manual and tests accordingly
