...
#   expect_success(show(correlation.heatmap))
# })
...
#   expect_success(show(contrast.heatmap))
# })
...
#   expect_success(show(counts.heatmap))
# })


test_that("method-show for class DEprot.enrichResult works", {
  expect_success(show(DEprot::test.toolbox$gsea.results))
})



test_that("method-show for class DEprot.pvalues works", {
  expect_success(show(check.pvalues(DEprot::test.toolbox$diff.exp.limma, contrast = 1)))
})


test_that("method-show for class DEprot.normality works", {
  expect_success(show(suppressMessages(check.normality(DEprot.object = DEprot::test.toolbox$dpo.imp))))
})
