...
#### Dropout curve
This is the panel that quantifies what the density plot shows qualitatively, and it deserves a longer explanation.
...
Three numbers summarize the fit, all available in `global.stats`:

* **`dropout.slope`**: the slope of the logistic model, on the logit scale. A negative slope means that the probability of a value being missing decreases as the abundance increases, which is what a detection limit produces. A slope close to zero describes missing values scattered over the whole intensity range, independently of the abundance.
* **`dropout.pvalue`**: the p-value of the slope. With several thousand proteins even a very shallow slope reaches significance, therefore the magnitude of the slope and the shape of the curve are more informative than the p-value alone.
* **`LOD50`**: the intensity at which the model predicts 50% of missing values, computed as `-intercept/slope`. It is an empirical estimate of the limit of detection of the experiment and it is drawn as a vertical dashed line. It is reported only when it falls inside the range of the observed intensities; when the dropout is shallow the 50% point lies far outside the data and an extrapolated value would be meaningless, so `NA` is returned instead.

A fourth number completes the picture: `intensity.shift.pvalue`, the one-sided Wilcoxon test comparing the mean intensity of the incomplete proteins with that of the complete ones, together with the two medians (`median.intensity.incomplete` and `median.intensity.complete`). The `show` method combines slope and p-value into a one-line interpretation of the dominant mechanism.

Two comparisons make the numbers actionable. First, `LOD50` against the threshold of the randomization pool (`tail.threshold`, the dashed line of the density panel): when the pool sits at or below the estimated detection limit, the values drawn by `randomize.missing.values` fall in the region where the instrument stops detecting, which is what the method assumes. A pool sitting well above `LOD50` means that the randomized values are higher than the censoring point and the fold changes of the MNAR proteins will be compressed; lowering `tail.percentage` corrects it. Second, the fraction of missing values that are MNAR-like (`perc.missing.MNAR`): a steep dropout curve combined with a low MNAR fraction means that most of the censored values are scattered rather than concentrated within a group, so they will be handled by the imputation and not by the randomization.

One caveat when reading the top of the plot. The mean intensity is computed on the observed values only, so a protein missing in most of the samples has its abundance estimated from the few measurements that survived, which are the ones that happened to be above the detection limit. This inflates the abundance of the highly incomplete proteins and flattens the right tail of the curve, so the real dropout is usually somewhat steeper than the fitted one. `LOD50` is best used as an internal reference for comparing datasets processed in the same way, rather than as a physical property of the instrument.
...
Every proteomics project starts with the same question, usually asked after the experiment has already been run: how many replicates are needed? The textbook answer, `power.t.test`, is written for a single comparison and returns a number that is far too small here, because a differential analysis is not one test but ten thousand of them, and the threshold that a protein has to cross is not 0.05 but whatever the multiple-testing correction leaves once the whole table has been adjusted. The two things are linked: the more responsive proteins there are, the more the correction can afford to relax, and the easier it becomes for each of them to be detected. A sample size computed while ignoring the link is not conservative, it is simply wrong in an unknown direction.
...
&alpha; depends on the power, the power depends on &alpha;, and the couple that satisfies both is found by root finding at each sample size. [Liu & Hwang (Bioinformatics, 2007)](https://doi.org/10.1093/bioinformatics/btl664) showed that the relation reduces to a closed form when a single effect size is assumed. The function `estimate.power` implements both: the responsive proteins can either keep the effect sizes they actually showed, or be collapsed onto one target fold change.

Nothing else is needed. The results table of any of the four differential functions already stores the log2-FoldChanges and the per-group standard deviations, which is exactly what the calculation reads, so no external power package has to be installed.
...
* The first panel is the average power as a function of the number of replicates per group, with the target as a horizontal line and the size of the current experiment as a vertical dotted one.
* The second is the quantity most people are actually asking about, the expected number of true discoveries, which saturates at `m1` and shows how much is gained by adding replicates rather than whether an abstract threshold is crossed.
* The third is the distribution of the standardized effect sizes on which the whole calculation rests, and it deserves a look before the other two are believed.
...
By default (`effect.size = "empirical"`) the proportion of null proteins &pi;<sub>0</sub> is estimated from the p-value distribution, the `m1` proteins with the smallest p-values are taken as the responsive ones, and the power is averaged over the effect sizes they showed. This uses all the information in the pilot experiment, and it inherits its optimism: those proteins were selected *because* they came out significant, so their effect sizes are the upper tail of a distribution rather than a sample from it. This is the winner's curse, and it becomes more severe as &pi;<sub>0</sub> approaches 1. The `min.effect.size` parameter blunts it by discarding the small effects, but does not remove it.

The alternative is to state the effect worth detecting instead of measuring it. `desired.FC` takes a fold change on the linear scale and divides `log2(desired.FC)` by a quantile of the observed pooled standard deviations, which turns the question into *"how many replicates do I need to see a 1.5-fold change on a protein of typical variability"*:
...
Setting `sd.quantile = 0.75` plans for a protein noisier than three quarters of the proteome, which is the version of the calculation that survives contact with a grant reviewer. The median is the default and describes the typical protein.
...
The second is the selection of the responsive set, discussed above.

The third concerns [`diff.analyses.proDA`](#diff_proDA) and is worth stating explicitly, because the function is otherwise the one that handles missing values most honestly. `estimate.power` reads the per-protein standard deviations, and those are computed on the observed values only. For a protein affected by dropout, the observed values are the upper part of its intensity distribution, so the standard deviation is estimated on a truncated sample and comes out too small. The number of replicates in which the protein was actually quantified is taken from the `n.detected.<group>` columns, so the group sizes are correct, but the dispersion is not. On a proDA object the full-table estimate should be read as an optimistic bound, and the complete-cases version above as the realistic one.

> Objects generated with `stat.test = "wilcoxon"` are accepted, since the standard deviations are stored regardless of the test. The calculation nonetheless describes the power of a *t*-test in the planned experiment; for normally distributed data a rank test needs roughly 5% more samples to reach the same power, so the estimate is mildly optimistic in that case too.

>Relation with `MSstats::designSampleSize`: `MSstats` answers the same question with the same logic, converting the FDR into a per-test &alpha; under an assumed ratio between null and responsive features and then solving the two-sample non-central *t* equation. The difference is that it collapses the problem onto one `desiredFC` and one standard deviation taken from the fitted model, which is precisely the `desired.FC` mode described above.


<br>

## Extrapolating with `power4peaks`
`estimate.power` is deliberately parametric: it assumes the effect sizes observed in the pilot and asks what a larger experiment would do with them.<br>A complementary approach is to reconstruct the whole density of the effect sizes from the distribution of the test statistics, which is what [`power4peaks`](https://sebastian-gregoricchio.github.io/power4peaks/) does through the deconvolution of [Ferreira & Zwinderman (The Int. J. of Biostat., 2006)](https://doi.org/10.2202/1557-4679.1018). Any `DEprot.analyses` object can be converted directly:
...
The conversion covers all the differential functions of `DEprot`: the moderated and ordinary *t* statistics of `diff.analyses`, `diff.analyses.limma`, `diff.analyses.prolfqua` and `diff.analyses.proDA` are passed as such, while the rank statistics of a Wilcoxon test are converted into signed normal deviates through the p-values. For `proDA` the group sizes are rescaled to the effective sample size estimated by the dropout model (`n.approx`), since that, and not the nominal number of samples, is the amount of information the statistics were computed on.

The two answers should be read together rather than one against the other. They disagree when &pi;<sub>0</sub> is poorly determined, which is itself informative, and a difference of one or two replicates between them is a fair description of the uncertainty of the whole exercise.

> Neither method models the other half of the problem: in LFQ proteomics, adding samples does not only increase the power per protein, it also increases the number of proteins that pass the missing-value filter and reach the test at all. Both curves therefore describe the power over the proteins that were quantified in the pilot experiment, and both under-estimate what a larger experiment would actually return.
