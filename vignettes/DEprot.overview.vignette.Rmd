...
To overcome this possibility, it is possible to use the function `randomize.missing.values` to assign - only to completely missing proteins in a certain condition - random values from the bottom distribution of the full dataset. It will be sufficient to indicate the ID of a column belonging to the metadata which indicates the groups/conditions and the percentage of distribution from which select random numbers ("tail.percentage").
...
Hereafeter, an example in how to use the `missForest` method to impute the data:
</div>
...
Good practice is to visualize the distribution of the p-values and compare it with the adjusted ones in order to estimate the quality of the statistical tools used.<br>
