...
| *method*           | String indicating the correlation method used (one among: pearson, spearman, kendal)                    |
...
Unlike the PCA biplot, these arrows are the a-posteriori correlation between each protein and the coordinates (not the `prcomp` rotation coefficients), so they indicate the direction a protein follows in the ordination rather than an exact loading.
...
It works  with object of class `DEprot` and `DEprot.analyses`. In the case of the latter, also the differential analyses are re-performed using the same parameters (if possible).
...
Results can be visualized using `plot.NES` and `plot.GSEA`.<br>Other plots can be found in the output object (slots: `protein.network`, `pathway.network`, `NES.plot`, `dotplot_gene.ratio`); have a look to the `DEprot::test.toolbox$gsea.results` dummy example.
...
Some plots can be found in the output object slot. (slots: `protein.network`, `pathway.network`, `dotplot_gene.ratio`, `dotplot_fold.enrichment`); have a look to the `DEprot::test.toolbox$ora.results` dummy example.
...
| *affinity.propagation*               | if affinity propagation was applied via `simplify.enrichments`, here can be found the results   |

<br>

#### Reduce size of the enrichment by affinity propagation
The function `simplify.enrichments` applies the affinity propagation method developed by [Elizarraras *et al.*](https://doi.org/10.1093/nar/gkae456) and implemented in [WebGestalt](https://www.webgestalt.org/) online toolkit.<br>
